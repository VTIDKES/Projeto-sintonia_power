...
def _section_data_tables() -> None:
    ...
    with st.expander("Barras (Buses)", expanded=True):
        ...
        tables["buses"] = st.data_editor(
            tables["buses"], num_rows="dynamic", use_container_width=True
        )

    with st.expander("Geradores", expanded=False):
        st.write("Colunas: name, bus, p_mw, vm_pu, min_q_mvar, max_q_mvar, slack")
        tables["gens"] = st.data_editor(
            tables["gens"], num_rows="dynamic", use_container_width=True
        )

    with st.expander("Cargas", expanded=False):
        st.write("Colunas: name, bus, p_mw, q_mvar")
        tables["loads"] = st.data_editor(
            tables["loads"], num_rows="dynamic", use_container_width=True
        )

    with st.expander("Linhas", expanded=False):
        st.write("Use r/x/c/max_i ou std_type. Preferível: from_bus/to_bus.")
        tables["lines"] = st.data_editor(
            tables["lines"], num_rows="dynamic", use_container_width=True
        )

    with st.expander("Transformadores", expanded=False):
        st.write("Parâmetros aproximados (vk%, vkr%).")
        tables["trafos"] = st.data_editor(
            tables["trafos"], num_rows="dynamic", use_container_width=True
        )
...
def _section_simulation() -> None:
    ...
    with st.expander("Parâmetros", expanded=True):
        ...
        max_iter = col2.number_input(
            "Máx. iterações", min_value=1, max_value=100, value=20, step=1
        )
    ...
    if st.button("Executar fluxo de potência", type="primary"):
        ...
        if method in ("Newton-Raphson (pandapower)", "Comparar (ambos)"):
            try:
                ...
                nr = run_newton_raphson(
                    net, tol=float(tol), max_iter=int(max_iter), init=str(init)
                )
        ...
        if method in ("Simplificado (planilha)", "Comparar (ambos)"):
            slack_candidates = (
                tables["buses"][
                    tables["buses"]["type"].astype(str).str.upper() == "SLACK"
                ]["name"]
                .astype(str)
                .tolist()
            )
            ...
            vd = voltage_drop_and_losses(
                tables["buses"], tables["loads"], tables["lines"]
            )
    ...
    st.plotly_chart(
        fig_network(tables["buses"], tables["lines"], tables["trafos"]),
...
def _section_results() -> None:
    ...
    with tabs[0]:
        ...
        if not nr:
            ...
            st.plotly_chart(
                fig_voltage_profile(bus_res, nr.get("bus_name_map")),
                use_container_width=True,
            )
            st.plotly_chart(
                fig_angle_profile(bus_res, nr.get("bus_name_map")),
                use_container_width=True,
            )
            ...
            st.plotly_chart(
                fig_convergence(
                    nr.get("convergence_trace"),
                    nr.get("iterations"),
                    bool(nr.get("converged")),
                ),
                use_container_width=True,
            )
                ...
                [
                    {
                        ...
                        "P_loss_MW": nr.get("total_losses_mw", 0.0),
                        "Q_loss_MVAr": nr.get("total_losses_mvar", 0.0),
                    },
                    {
                        "método": "Simplificado",
                        "P_loss_MW": simp.get("voltage_drop", {}).get("total_losses_mw", 0.0),
...
- Newton-Raphson AC (pandapower)
- Métodos simplificados (DC + quedas/perdas)
