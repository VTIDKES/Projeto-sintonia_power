...
st.title("Desenho Interativo de Rede Elétrica")
...
if st.button("Gerar rede Pandapower"):
    net = pp.create_empty_network()
    # Adicionar barras
    barra_map = {}
    for idx, row in st.session_state.barras.iterrows():
        barra_map[row.name] = pp.create_bus(net, vn_kv=110 if row.y>0 else 20, name=row.name)

    # Ext_grid na primeira barra
    pp.create_ext_grid(net, bus=list(barra_map.values())[0], vm_pu=1.02)
    ...
    pp.runpp(net)
    st.success("Fluxo de carga calculado!")
    st.write(net.res_bus[["vm_pu", "va_degree"]])
    st.write(net.res_line[["loading_percent"]])
